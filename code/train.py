...
def prepare_data(src, target):
    nick_id, item_id, ts = src
    hist_item, hist_mask, his_ts = target
    return nick_id, item_id, hist_item, ts, his_ts, hist_mask
...
def evaluate_full(sess, test_data, model, model_path, batch_size, item_cate_map, save=True, coef=None):
    ...
    for src, tgt in test_data:
        nick_id, item_id, hist_item, ts, his_ts, hist_mask = prepare_data(src, tgt)
        ts = [sum(t)/len(t) for t in ts]
        user_embs = model.output_user(sess, [hist_item, ts, his_ts, hist_mask])
...
def get_model(dataset, model_type, item_count, batch_size, maxlen, ta,tmax,tmin):
    print(f'ta={ta}')
    print(f'tmax={tmax}')
    if model_type == 'DNN': 
        ...
        model = Model_MIND(item_count, args.embedding_dim, args.hidden_size, batch_size, args.num_interest, maxlen, ta, tmax,tmin, relu_layer=relu_layer)
    elif model_type == 'ComiRec-DR':
        model = Model_ComiRec_DR(item_count, args.embedding_dim, args.hidden_size, batch_size, args.num_interest, maxlen, 0, tmax,tmin)
...
def get_exp_name(dataset, model_type, batch_size, lr, maxlen,ta, save=True):
    # extr_name = input('Please input the experiment name: ')
    extr_name = str(int(time.time()))
    para_name = '_'.join([dataset, model_type, 'top'+str(args.topN), 'ta'+str(ta), 'b'+str(batch_size), 'lr'+str(lr), 'd'+str(args.embedding_dim), 'len'+str(maxlen)])
...
def train(
    ...
    exp_name = get_exp_name(dataset, model_type, batch_size, lr, maxlen,ta)
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        train_data = DataIterator(train_file, batch_size, item_cate_map, maxlen, train_flag=0)
        valid_data = DataIterator(valid_file, batch_size, item_cate_map, maxlen, train_flag=1)
        
        model = get_model(dataset, model_type, item_count, batch_size, maxlen, ta, tmax, tmin)
        ...
        try:
            ...
            for src, tgt in train_data:
                ...
                loss = model.train(sess, list(data_iter) + [tmp_lr])
                if iter % test_iter == 0 and iter > 1000000:
                    tmp_lr = round(0.99 * tmp_lr, 6)
                    print('tmp learning rate: ', tmp_lr)
                loss_sum += loss
                iter += 1
...
def test(
    ...
    exp_name = get_exp_name(dataset, model_type, batch_size, lr, maxlen,ta, save=False)
    best_model_path = "best_model/" + exp_name + '/'
    gpu_options = tf.GPUOptions(allow_growth=True)
...
if __name__ == '__main__':
    ...
    if args.dataset == 'taobao':
        path = '/home/wangzhikai/work/playground/recommendation_learning/dataset/tianchi/'
        item_count = 1708531
        batch_size = 256
        maxlen = 50
        test_iter = 500
        ...
        path = '/home/wangzhikai/work/playground/recommendation_learning/dataset/ComiRec/book_data/'
        item_count = 367983
        batch_size = 128
        maxlen = 20
        test_iter = 1000
    elif args.dataset == 'video':
        path = '/home/wangzhikai/work/playground/recommendation_learning/dataset/amazon/'
        item_count = 0
        batch_size = 128
        maxlen = 20
        test_iter = 1000
    elif args.dataset == 'book3':
        path = '/home/wangzhikai/work/playground/recommendation_learning/dataset/amazon/'
        item_count = 367983
        batch_size = 128
        maxlen = 20
        ...
        maxlen = 20
        test_iter = 1000
    elif args.dataset == 'musical':
        path = '/home/wangzhikai/work/playground/recommendation_learning/dataset/amazon/'
        item_count = 901
        batch_size = 128
        maxlen = 20
        test_iter = 1000
    elif args.dataset == 'ml10':
        path = '/home/wangzhikai/work/playground/recommendation_learning/dataset/movielens/ml-10M100K/'
        item_count = 10678
        batch_size = 128
        maxlen = 30
        test_iter = 1000
        ...
        path = '/home/wangzhikai/work/playground/recommendation_learning/dataset/movielens/ml-20m/'
        item_count = 26745
        batch_size = 128
        maxlen = 30
        test_iter = 1000
        maxtime = 1427784002
        mintime = 789652004
    elif args.dataset == 'ml25':
        path = '/home/wangzhikai/work/playground/recommendation_learning/dataset/movielens/ml-25m/'
        item_count = 59048
        batch_size = 128
        maxlen = 30
        test_iter = 1000
    ...
    if args.p == 'train':
        train(train_file=train_file, valid_file=valid_file, test_file=test_file, cate_file=cate_file, 
              ...
              model_type=args.model_type, lr=args.learning_rate, max_iter=args.max_iter, patience=args.patience,ta=args.ta,tmax=maxtime,tmin=mintime)
    elif args.p == 'test':
